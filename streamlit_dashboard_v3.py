...
def fetch_data(query, params=None):
    try:
        ...
        data = cur.fetchall()
        conn.close()
        df = pd.DataFrame(data, columns=columns) if data else pd.DataFrame()
...
def calculate_demand_score(batch_sales, batch_weights, batch_expenses, batch_size, batch_start_date, batch_end_date, all_batches):
    ...
    if batch_sales.empty:
        return 0, {'outcome': 0, 'structure': 7, 'concentration': 0}, {}
    ...
    regular_count = len(batch_buyers) // 2  # Simple proxy
...
with tabs[0]:
    ...
    <div class="header-warning">
        ...
        Demand Score: {int(demand_score)}/100 {status_color} {status_text}<br>
        Outcome: {int(score_breakdown.get('outcome', 0))}/60 | Structure: {int(score_breakdown.get('structure', 0))}/40<br>
        Concentration Risk: {score_breakdown.get('concentration', 0):.1f}% from top 3 buyers
    </div>
    """, unsafe_allow_html=True)
